...
export const runNodeContainer = (
  ...
  const container = new GenericContainer('ghcr.io/midnight-ntwrk/midnight-node:0.21.0-rc.5')
    .withEnvironment({
      CFG_PRESET: 'dev',
...
export const runProofServerContainer = (
  ...
  const container = new GenericContainer('ghcr.io/midnight-ntwrk/proof-server:7.0.2')
    .withEnvironment({
      RUST_BACKTRACE: 'full',
    })
...
export const runTxGenerator = (
  ...
  const container: GenericContainer = new GenericContainer(
    'ghcr.io/midnight-ntwrk/midnight-node-toolkit:0.21.0-rc.5',
  ).withCommand(
    [
      'generate-txs',
      '--dust-warp',
