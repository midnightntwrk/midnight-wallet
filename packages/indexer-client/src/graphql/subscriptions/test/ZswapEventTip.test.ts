...
import type { ZswapEventTipSubscription, ZswapEventTipSubscriptionVariables } from '../../generated/graphql.js';
import { ZswapEventTip } from '../ZswapEventTip.js';
...
const eventTip: ZswapEventTipSubscription = {
  zswapLedgerEvents: { id: 41, maxId: 97 },
};
 ...
 *
 * @remarks
 *   Two fields and no more. `maxId` is the answer — how far the event timeline goes — and `id` is what makes the answer
 *   checkable against the cursor that asked. `raw` is deliberately absent: the caller never deserializes this event,
 *   and asking for its bytes would pull a payload across the wire on every poll for nothing.
 */
const expectedDocument = `subscription ZswapEventTip($id: Int) {
...
describe('ZswapEventTip subscription', () => {
  it('should execute the zswapLedgerEvents operation asking only for the id and the timeline length', async () => {
    const variables: ZswapEventTipSubscriptionVariables = { id: 40 };

    const { recorded, result } = await Effect.gen(function* () {
      const requests = yield* Ref.make<readonly RecordedRequest[]>([]);
      const result = yield* ZswapEventTip.run(variables).pipe(
        Stream.runCollect,
  ...
  it('should support subscription function injection', async () => {
    const variables: ZswapEventTipSubscriptionVariables = { id: null };
    const seen = Effect.runSync(Ref.make<readonly ZswapEventTipSubscriptionVariables[]>([]));
    const requests = Effect.runSync(Ref.make<readonly RecordedRequest[]>([]));
    const injected = (received: ZswapEventTipSubscriptionVariables) =>
      Stream.unwrap(Ref.update(seen, (all) => [...all, received]).pipe(Effect.as(Stream.make(eventTip))));

    await Effect.gen(function* () {
      expect(Chunk.toArray(yield* Stream.runCollect(ZswapEventTip.run(variables)))).toEqual([eventTip]);
      ...
      Effect.provideService(ZswapEventTip.tag, injected),
      Effect.provide(recordingSubscriptionClient(requests)),
      Effect.mapError((err) => `Encountered unexpected error: ${err.message}`),
      Effect.runPromise,
    );
