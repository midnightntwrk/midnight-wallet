 ...
 *
 * @remarks
 *   `forkSimulation.test.ts` proves the crossing — hand-over, the state arriving whole, a ledger-v9 payment, a spend —
 *   and needs no ledger translation to do it, because `translationStub.ts` reconstructs ledger-v9 from the same coins
 *   the ledger-v8 chain was paid. What it cannot prove is that the reconstruction is faithful: the chain the wallet's
 *   tree is compared against is one the suite built itself.
 *
 *   This closes that gap against the ledger team's real v8-to-v9 state translation. The ledger-v8 chain hands its own
 *   serialized ledger to the translation and the ledger-v9 chain continues from the result — the chain's own account of
 ...
 *       ledger-v9 recognises it only if that path resolves to a root the translated state holds. So a wallet whose
 *       state crossed as bytes transacts against the real translation. That is the money test, and nothing short of a
 *       real translation can support it.
 *
...
import { type MintedCoin, mintable, v8Payment, simulatedChainRoot } from './translationStub.js';
...
/** The ledger-v8 commitment sequence: three to us, a stranger's, ours, a stranger's again. */
...
describe('the two ledger versions agree on what the wallet owns', () => {
  it('derives the same public keys from one seed, which is what lets a carried coin be re-inserted as itself', () => {
    ...
    expect(v9Keys.coinPublicKey).toBe(v8Keys.coinPublicKey);
    expect(v9Keys.encryptionPublicKey).toBe(v8Keys.encryptionPublicKey);
    expect(ledgerV9.shieldedToken().raw).toBe(ledgerV8.shieldedToken().raw);
  });

  it('computes identical coin commitments for identical coins', () => {
    const coin = mintable(ledgerV8.shieldedToken().raw, 100n, walletRecipient());
    const plain = { type: coin.type, nonce: coin.nonce, value: coin.value };

    expect(ledgerV9.coinCommitment(plain, coin.recipient.coinPublicKey)).toBe(
      ledgerV8.coinCommitment(plain, coin.recipient.coinPublicKey),
    );
  });
});
...
describe('a shielded wallet crossing a byte-faithful hard fork', () => {
  it('arrives holding the truly translated tree, and spends against it', async () =>
    Effect.gen(function* () {
      ...
      // --- ledger-v8: a real ledger-v8 chain, synced by the ledger-v8 variant ------------------------------------
      yield* Effect.forEach(coins, (coin) => fork.v8.submitTransaction(v8Payment(networkId, coin)), {
        discard: true,
      });
      const v1State = yield* wallet.awaitState((state) => totalValue(state.state) === walletTotal);
      expect(yield* wallet.activeTag).toBe(V1Tag);
      expect(coinIndices(v1State.state)).toEqual(walletIndices);
      expect(treeSize(v1State.state)).toBe(treeSizeAtFork);
      const v8Root = merkleRoot(v1State.state);
      ...
      // The translation preserved the tree the V1 wallet was looking at. Asserted before the wallet is involved
      // so that a translation regression is distinguishable from a wallet one.
      expect(translatedRoot).toBe(v8Root);
      ...
      // the height the ledger-v8 chain reached.
      ...
      // Parked on the boundary, not rewound: the ledger-v9 timeline continues the indexer's event ids.
      ...
      expect(coinValues(v2State.state)).toEqual([...walletValues]);
      expect(coinIndices(v2State.state)).toEqual(walletIndices);
      expect(treeSize(v2State.state)).toBe(treeSizeAtFork);
      expect(awaitingCoinHashes(v2State.state)).toBe(false);

      // **The fidelity link.** The wallet's tree was decoded out of ledger-v8's bytes by the ledger-v9
      // ledger; the chain's was produced by the ledger team's own state translation from the same ledger-v8.
      // Two independent crossings of one tree, and they agree down to the root.
      expect(merkleRoot(v2State.state)).toBe(translatedRoot);
      ...
      expect(v2State.state.progress.appliedIndex).toBe(forkBlock + 1n);
      ...
      const spend = carried<ledgerV9.UnprovenTransaction>(transfer, forkVersion).eraseProofs();
