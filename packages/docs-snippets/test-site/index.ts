...
const INDEXER_PORT = Number.parseInt(globalThis.process?.env?.['INDEXER_PORT'] ?? '8088', 10);
const NODE_PORT = Number.parseInt(globalThis.process?.env?.['NODE_PORT'] ?? '9944', 10);
...
const { wallet } = await initWalletWithSeed(Buffer.from(seed));
