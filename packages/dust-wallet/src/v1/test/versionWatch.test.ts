...
/**
 * Noticing a protocol version the dust event timeline never mentions — on the variant that has to notice this fork.
 *
 * @remarks
 *   This is the ledger-v8 side, and therefore the side the drill caught out. A dust wallet learns which version the chain
 *   is on from the events it is served, and the dust event subscription carries no progress arm — it says nothing at
 *   all when there is nothing to say. On a chain that forks and then produces no dust traffic, that wallet is never
 *   told: it stays on the variant it was running, and everything the facade builds through it stays routed to that
 *   variant's ledger. That is what the version watcher removes, by asking the chain its version on a timer and feeding
 *   the answer into the same `annotateVersion` path the events use.
 *
...
> => {
  ...
  return {
    ...
    syncCapability: makeDefaultSyncCapability(),
...
describe('a dust wallet on the quiet side of a fork', () => {
  it('crosses on the watcher alone, without a single event ever arriving', async () => {
    // The drill, reproduced: a wallet caught up to the tip of a chain that has forked and then produced no dust traffic
    // at all. Against a wallet with no version watcher this never resolves — the source has nothing to say and the
    // version is never observed, so the variant never asks to hand over.
    const observed = await Effect.gen(function* () {
      const stateRef = yield* SubscriptionRef.make(syncedWallet(41n));
      const scope = yield* Scope.make();
      const variant = new RunningV1Variant(scope, { stateRef, activationRange: activeRange }, variantContextOf());
      ...
      yield* Effect.forkScoped(
        variant
          .startSync(fixtureSecretKey())
          .pipe(Stream.runScoped(Sink.drain), Effect.provideService(Scope.Scope, scope)),
      );
