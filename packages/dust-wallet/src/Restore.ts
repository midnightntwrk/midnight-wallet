...
export class UnsupportedSnapshotVersionError extends Data.TaggedError(
  '@midnightntwrk/wallet-sdk-dust-wallet/Restore/UnsupportedSnapshotVersionError',
)<{
...
/**
 * Reads the protocol version a serialized dust wallet snapshot declares.
 *
...
/**
 * Chooses the variant that should read a serialized dust wallet snapshot.
 *
 * @remarks
      ...
      Either.fromOption(
        ...
        () =>
          new UnsupportedSnapshotVersionError({
            message: `No registered variant reads dust wallet snapshots of protocol version ${protocolVersion}.`,
            protocolVersion,
          }),
      ),
  });
