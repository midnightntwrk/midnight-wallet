...
export type DustWalletAPI<TStartAux = DustSecretKey, TSerialized = string> = {
   ...
   * is quiet. Tracks the same quantity used as `allow_fee_payment` for the registration, so pairing with
   ...
   * @param requiredAmount - Threshold to wait for, as a Dust amount. Returns immediately if `<= 0n`.
   * @param opts.now - Source of current time, called on every tick. Defaults to `() => new Date()`. Facade callers
   *   should pass `() => this.clock.now()` so simulator-driven tests respect simulator time.
   * @param opts.timeoutMs - Reject after this many ms with no progress. Default `300_000`.
   */
        ...
        rx.combineLatest([this.state, rx.timer(0, 1000)]).pipe(
          rx.filter(([dustState]) => {
            ...
            const maxGeneratedNow = dustState
              .estimateDustGeneration(nightUtxos, now())
