 ...
 *   registers Night for dust generation, the V1 variant syncs the events it produced, the timeline reaches the boundary
 *   version, the wallet hands over to the ledger-v9 variant with a state holding _no dust_, and that variant finds the
 *   same dust again by syncing the indexer's replay.
 *
 ...
 *   `dustInitialUtxo` event identically apart from its header — so the replay here is the same event bytes, and the V2
 *   wallet's dust is comparable to the V1 wallet's directly, UTXO for UTXO and root for root. The `describe` block
 *   below establishes exactly that, and it is the load-bearing assumption of the whole file: if a ledger release ever
 *   changes the encoding, it fails and the model is retired rather than quietly becoming a fiction.
 *
 ...
 *   and this harness has no ledger-v9 chain — the replay is an event log, which is what an indexer serves and all a
 *   fork's aftermath consists of. What stands in its place is stronger than the spend on one axis and weaker on
 *   another: the V2 wallet's own commitment-tree root is asserted equal to the V1 wallet's, so the tree the path would
 *   be built against is byte-identical, but no ledger is asked to accept anything.
 */
...
import {
  DustSecretKey as V9SecretKey,
  LedgerParameters as V9LedgerParameters,
  Event as V9Event,
} from '@midnightntwrk/ledger-v9';
import { NetworkId, ProtocolVersion } from '@midnightntwrk/wallet-sdk-abstractions';
import { Deferred, Effect, Option, Queue, Stream } from 'effect';
import { describe, expect, it, vi } from 'vitest';
import { CoreWallet as V1CoreWallet } from '../v1/CoreWallet.js';
import { V1Tag } from '../v1/RunningV1Variant.js';
import { DUST_EVENT_COUNT, buildDustChain, dustSeed, freshWallet as freshV1Wallet } from '../v1/test/dustEvents.js';
import { CoreWallet as V2CoreWallet } from '../v2/CoreWallet.js';
import { V2Tag } from '../v2/RunningV2Variant.js';
import { dustParameters as v9DustParameters, freshWallet as freshV2Wallet } from '../v2/test/dustEvents.js';
import { type TimelineEvent, numberedFrom, reframeAsV9 } from './forkReplay.js';
 ...
 *
 * `forkVersion` is where the V2 variant is registered _and_ the version the indexer reports the replayed events under —
 * the same number reaching the runtime through registration and through the timeline, which is the whole point of D5.
 * It is deliberately an arbitrary number: the real fork's protocol version is not final, and nothing here may depend on
 * it.
 */
const forkVersion = ProtocolVersion.ProtocolVersion(7n);

/** A version bump the V1 variant owns: inside `[MinSupportedVersion, forkVersion)`, so it must not migrate. */
const withinRangeVersion = ProtocolVersion.ProtocolVersion(5n);

/** What the indexer reports the ledger-v8 history under. */
const v8Version = Number(ProtocolVersion.MinSupportedVersion);
 ...
 *   id it had reached when the fork happened. This is therefore also where a migrated wallet's parked cursor is
 *   waiting.
 */
...
/** The ledger-v8 history as the indexer served it: the fixture's events, numbered from one, at the ledger-v8 version. */
const v8History = (eventBytes: readonly Uint8Array[]): readonly TimelineEvent[] =>
  numberedFrom(eventBytes, 1, v8Version);

/** The replay: the same events again, renumbered from the boundary and reported at the ledger-v9 version. */
...
/**
 * What a wallet that read this whole history holds, computed by ledger-v8 and nothing else.
 *
 * @remarks
 ...
 *   them: `CoreWallet` folding the events directly, with no variant, no runtime and no fork anywhere near it. A V2
 *   wallet that agrees with this agrees with a reading the fork machinery had no hand in producing.
 */
const walletOfHistory = (eventBytes: readonly Uint8Array[], at: Date): V1CoreWallet => {
  const [wallet] = V1CoreWallet.applyEventsWithChanges(
    freshV1Wallet(),
    V8SecretKey.fromSeed(dustSeed()),
    eventBytes.map((bytes) => V8Event.deserialize(bytes)),
    at,
  );
  return wallet;
};
...
describe('the two ledger versions agree on what a dust event says', () => {
  it('derives the same dust public key from one seed, which is what makes the replayed dust the same dust', () => {
    ...
    // replay would be somebody else's dust and the V2 wallet would end up empty.
    const seed = dustSeed();

    expect(V9SecretKey.fromSeed(seed).publicKey).toBe(V8SecretKey.fromSeed(seed).publicKey);
  });

  it('values dust against the same initial parameters', () => {
    // Dust's local state is parameterised where shielded's is not, and the migration supplies the ledger-v9 parameters
    // rather than carrying the ledger-v8 ones. Comparing balances across the boundary only means something because the
    // two sets agree.
    const v8Parameters = V8LedgerParameters.initialParameters().dust;
    const v9Parameters = V9LedgerParameters.initialParameters().dust;

    expect(v9Parameters.nightDustRatio).toBe(v8Parameters.nightDustRatio);
    expect(v9Parameters.generationDecayRate).toBe(v8Parameters.generationDecayRate);
    expect(v9Parameters.dustGracePeriodSeconds).toBe(v8Parameters.dustGracePeriodSeconds);
  });
  ...
  it('refuses the other version framing, which is why a replayed event has to be re-framed at all', async () => {
    ...
    expect(() => V9Event.deserialize(chain.eventBytes[0])).toThrowError(/midnight:event/);
  });

  it('builds the same dust from a re-framed event as from the original', async () => {
    const chain = await buildDustChain();

    const v1Wallet = walletOfHistory(chain.eventBytes, chain.syncTime);
    const [v2Wallet] = V2CoreWallet.applyEventsWithChanges(
      freshV2Wallet(),
      V9SecretKey.fromSeed(dustSeed()),
      chain.eventBytes.map((bytes) => V9Event.deserialize(reframeAsV9(bytes))),
      chain.syncTime,
    );
    ...
    expect(dustIdentities(v2Wallet)).toEqual(dustIdentities(v1Wallet));
    expect(commitmentTreeRoot(v2Wallet)).toBe(commitmentTreeRoot(v1Wallet));
    expect(generationTreeRoot(v2Wallet)).toBe(generationTreeRoot(v1Wallet));
    expect(balanceAt(v2Wallet, chain.syncTime)).toBe(balanceAt(v1Wallet, chain.syncTime));
  });
});
...
describe('a dust wallet crossing a hard fork', () => {
  it(
    ...
    async () =>
      Effect.gen(function* () {
        ...
        // --- ledger-v8: the ledger-v8 variant syncs the ledger-v8 chain's events -----------------------------------
        yield* Queue.offer(wire, v8History(chain.eventBytes));

        const v1State = yield* wallet.awaitState((state) => dustCount(state.state) === DUST_EVENT_COUNT);
        expect(yield* wallet.activeTag).toBe(V1Tag);
        expect(dustIndices(v1State.state)).toEqual(dustIndicesAtFork);
        expect(v1State.state.progress.appliedIndex).toBe(BigInt(DUST_EVENT_COUNT));
        // Everything the V1 wallet knows, recorded before the hand-over destroys the ledger-v8 objects holding it.
        const v8Dust = dustIdentities(v1State.state);
        const v8CommitmentRoot = commitmentTreeRoot(v1State.state);
        const v8GenerationRoot = generationTreeRoot(v1State.state);
        const v8Balance = balanceAt(v1State.state, chain.syncTime);
        expect(v8CommitmentRoot).toBeDefined();
        expect(v8Balance).toBeGreaterThan(0n);

        // --- the fork: the indexer's first ledger-v9 event reaches the still-open ledger-v8 subscription ------------
        // It is the replay's own opening event, delivered in the encoding the V1 variant can read. The ledger-v8
        ...
        // What the V1 variant handed over: its identity, the version that triggered the hand-over, and a cursor
        // parked on the last event it applied — the boundary event was observed and annotated, deliberately not applied.
        expect(migration.from.dustPublicKey).toBe(V8SecretKey.fromSeed(dustSeed()).publicKey);
        ...
        expect(migration.to.dustPublicKey).toBe(V9SecretKey.fromSeed(dustSeed()).publicKey);
        ...
        expect(migration.to.nightDustRatio).toBe(v9DustParameters().nightDustRatio);

        // --- ledger-v9: the dust is re-discovered, not carried -----------------------------------------------------
        const v2State = yield* wallet.awaitState(
        ...
        expect(dustIdentities(v2State.state)).toEqual(v8Dust);
        // And the same trees, which is what a spend's Merkle path would be built against.
        expect(commitmentTreeRoot(v2State.state)).toBe(v8CommitmentRoot);
        expect(generationTreeRoot(v2State.state)).toBe(v8GenerationRoot);
        // Valued at the same instant on both sides, because dust generates and decays and a balance means nothing
        // without one.
        expect(balanceAt(v2State.state, chain.syncTime)).toBe(v8Balance);
        ...
        expect(v2State.state.progress.appliedIndex).toBe(BigInt(replayEndId));
  ...
  it(
    ...
    async () =>
      Effect.gen(function* () {
        // Same timeline, same registration, one number different: the indexer reports a version the V1 variant
        ...
        yield* Queue.offer(wire, reportedAt(v8History(chain.eventBytes), withinRangeVersion));
  ...
  it(
    ...
    async () =>
      Effect.gen(function* () {
        // Scenario 2: a wallet started from seed after the v9 fork has happened. Its V1 variant meets the whole
        // ledger-v8 history and the boundary in a single batch, so the split happens *within* one update rather than
        // across two.
        const chain = yield* Effect.promise(() => buildDustChain());
        const replay = replayOf(chain.eventBytes);
        const wire = yield* Queue.unbounded<readonly TimelineEvent[]>();
        ...
        yield* Queue.offer(wire, [...v8History(chain.eventBytes), replay[0]]);
        ...
        expect(yield* wallet.activeTag).toBe(V2Tag);
        // Compared against the history folded straight into a `CoreWallet`, because this run never exposes a ledger-v8
        ...
        expect(dustIdentities(v2State.state)).toEqual(dustIdentities(expected));
        expect(commitmentTreeRoot(v2State.state)).toBe(commitmentTreeRoot(expected));
        expect(balanceAt(v2State.state, chain.syncTime)).toBe(balanceAt(expected, chain.syncTime));
        expect(v2State.state.progress.appliedIndex).toBe(BigInt(replayEndId));
      }).pipe(Effect.scoped, Effect.runPromise),
    60_000,
  );
