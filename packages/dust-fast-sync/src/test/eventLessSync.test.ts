...
import { DustSecretKey, LedgerParameters, type Event as LedgerEvent } from '@midnight-ntwrk/ledger-v8';
import {
  DustSecretKey as RcDustSecretKey,
  LedgerParameters as RcLedgerParameters,
  dustFirstNonce,
  dustNullifier,
} from '@midnight-ntwrk/ledger-v8-rc';
import { NetworkId } from '@midnightntwrk/wallet-sdk-abstractions';
import { DustNullifierTransactions } from '@midnightntwrk/wallet-sdk-indexer-client';
import type { DustNullifierTransactionsSubscriptionVariables } from '@midnightntwrk/wallet-sdk-indexer-client';
...
import {
  ...
  makeProjectionsIndexerService,
  nullifierPhaseProgress,
} from '../EventLessSync.js';
import {
...
import { toRcState } from '../StateOps.js';
...
describe('V1 projections nullifier subscription', () => {
  const ledgerParametersHex = Buffer.from(RcLedgerParameters.initialParameters().serialize()).toString('hex');
  ...
  it('keeps exact matches in fixed-width LE form, including nullifiers with a zero top byte', async () => {
    ...
    const service = makeProjectionsIndexerService({
...
describe('V1 projections dust spend resolution', () => {
  const initialParameters = RcLedgerParameters.initialParameters();
  ...
  it("skips dust spends whose nullifier is not this wallet's (multi-party transactions)", async () => {
    const secretKey = DustSecretKey.fromSeed(Buffer.from(seedHex, 'hex'));
    const rcSecretKey = RcDustSecretKey.fromSeed(Buffer.from(seedHex, 'hex'));
    const state = CoreWallet.initEmpty(dustParameters, secretKey, networkId);
    const foreignNullifier = 123456789n;

    const updates = await Effect.runPromise(
      createDustUtxoUpdates(
        toRcState(state.state),
  ...
  it('resolves an owned spend into a spent update and its successor', async () => {
    ...
    const qdo = {
      ...
      owner: rcSecretKey.publicKey,
      nonce: dustFirstNonce(backingNight, rcSecretKey.publicKey),
      seq: 0,
      ctime: new Date(1_000_000),
      backingNight,
      mtIndex: 4n,
    };
    const nullifier = dustNullifier(qdo, rcSecretKey);
    const knownUtxos = DustUtxoMap.create([
      {
        dustNullifier: nullifier,
        genInfo: { value: 5_000_000_000n, owner: rcSecretKey.publicKey, nonce: backingNight, dtime: undefined },
        generationMtIndex: 0,
        qdo,
        transactionId: 7,
