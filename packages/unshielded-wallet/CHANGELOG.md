...
- Updated dependencies [db9102e]
  ...
  - @midnightntwrk/wallet-sdk-runtime@1.1.0-rc.3
...
- 59e9260: fix(unshielded): reject out-of-order sync updates and unknown spends instead of applying them
...
- 2e616b1: feat: support BridgeClaimTransaction type in unshielded wallet sync schemaw
- 3ee8186: fix: treat BridgeClaimTransaction as successful implicitly
...
- 89d54b6: Enforce signature-scheme consistency (schnorr vs ecdsa) for unshielded wallets. A new `SchemeMismatchError`
  and pure guards reject mixed schemes early: signing rejects a signature whose scheme differs from the input owners
  before it is attached (no partially-signed transaction reaches the network), and deserialization rejects a snapshot
  whose stored address does not derive from its verifying key (a relabelled key whose encoding does not match its tag
  fails to decode).
...
- Updated dependencies [2c4a115]
  - @midnightntwrk/wallet-sdk-unshielded-state@1.0.0-beta.11
...
- Updated dependencies [646c8df]
  ...
  - @midnightntwrk/wallet-sdk-unshielded-state@1.0.0-beta.10
...
- Updated dependencies [2a0d132]
  - @midnightntwrk/wallet-sdk-unshielded-state@1.0.0-beta.9
...
- Updated dependencies [ae22baf]
  ...
  - @midnightntwrk/wallet-sdk-unshielded-state@1.0.0-beta.8
