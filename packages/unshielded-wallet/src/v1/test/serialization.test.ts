...
const initEmpty = (seedHex: string): CoreWallet => {
  const keystore = createKeystore(Buffer.from(seedHex, 'hex'), networkId);
  return CoreWallet.init(PublicKey.fromKeyStore(keystore), networkId);
};
...
describe('V1 unshielded wallet serialization', () => {
  it.each([
    ...
    const restored = pipe(capability.deserialize(firstIteration), EitherOps.getOrThrowLeft);
  ...
  it('returns Left with OtherWalletError for invalid JSON strings', () => {
    ...
    fc.assert(
      fc.property(fc.string(), (invalidJson) => {
        const result = capability.deserialize(invalidJson);
  ...
  it('returns Left with OtherWalletError for valid JSON that does not match the schema', () => {
    ...
    fc.assert(
      fc.property(fc.json(), (randomJsonValue) => {
        const result = capability.deserialize(randomJsonValue);
