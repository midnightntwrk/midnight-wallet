...
/**
 * Noticing a protocol version the address's own timeline never mentions — on the variant that has to notice this fork.
 *
 * @remarks
 *   This is the pre-fork side, and therefore the side the drill caught out. An unshielded wallet learns which version the
 *   chain is on from the transactions it is served, and it is served only the transactions that touch its own address.
 *   The subscription's progress arm carries no version — it reports how far the address's timeline goes and nothing
 *   else — so on a chain that forks and then produces no traffic for this address, the wallet is never told. That is
 *   what the version watcher removes, by asking the chain its version on a timer and feeding the answer into the same
 *   `annotateVersion` path the transactions use.
 *
...
const variantContextOf = (): RunningV1Variant.Context<string, WalletSyncUpdate> => {
  const configuration = { indexerClientConnection, networkId, versionWatch: { intervalMs: 20 } };
...
describe('an unshielded wallet on the quiet side of a fork', () => {
  it('crosses on the watcher alone, without a single transaction ever arriving', async () => {
    // The drill, reproduced: a wallet caught up to the tip of a chain that has forked and then produced no traffic for
    // this address at all. Against a wallet with no version watcher this never resolves — the source has nothing to
    // say and the version is never observed, so the variant never asks to hand over.
