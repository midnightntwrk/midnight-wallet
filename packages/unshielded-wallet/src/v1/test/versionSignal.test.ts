 ...
 *
 * @remarks
 *   This is the ledger-v8 side, and therefore the side the drill caught out. An unshielded wallet learns which version
 *   the chain is on from the transactions it is served, and it is served only the transactions that touch its own
 *   address. On a chain that has forked and then produced no traffic for this address, that is nothing at all, and the
 *   wallet would stay on the variant it was running with everything built through it routed to that variant's ledger.
 *
