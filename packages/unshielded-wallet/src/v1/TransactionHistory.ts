...
export const makeSimulatorTransactionHistoryService = (): TransactionHistoryService<UnshieldedUpdate> => {
  const txHistoryStorage = new InMemoryTransactionHistoryStorage<UnshieldedTransactionHistoryEntry>();
  ...
  return {
    ...
    serialize: async (): Promise<SerializedUnshieldedTransactionHistory> => {
      return serializeUnshieldedTransactionHistoryStorage(txHistoryStorage);
    },
  };
};
