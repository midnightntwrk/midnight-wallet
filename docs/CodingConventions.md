# Coding Conventions — Midnight Wallet SDK

Human-facing reference for the functional programming conventions this codebase follows. This is the single source of
truth for the _why_ and the worked examples; the distilled hard rules that coding agents load automatically live in
`.claude/rules/functional-style.md` — keep the two in sync when conventions change.

Related reading:

- [ADR 0004](decisions/0004-use-effect.md) — why Effect was chosen (over fp-ts, plain RxJS, lodash/fp)
- [ADR 0006](decisions/0006-structure-for-flexibility-and-robustness.md) — architecture and Effect usage boundaries
- [`Design.md`](Design.md) — overall architecture with diagrams
- [effect.website](https://effect.website/) — Effect framework fundamentals (not duplicated here)
...
Per [ADR 0006](decisions/0006-structure-for-flexibility-and-robustness.md):
...
**Import aliases** — Effect modules that shadow JS globals must be imported under an alias (enforced by the language
service, see `tsconfig.base.json`):
