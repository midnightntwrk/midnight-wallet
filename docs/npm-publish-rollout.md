...
One-time playbook for moving CD from GitHub Packages → npmjs with Trusted
Publishing + provenance + environment gating. Delete or convert to a
steady-state doc once items below are checked off.
...
For each of the 14 publishable packages (see list at bottom), configure
**two** pending Trusted Publishers via npmjs.com → package or org settings:

- Provider: GitHub Actions
- Owner/Repo: `<owner>/<this-repo>`
- Workflow filename: `cd.yml`
- Environment: `npm-publish-stable` *(entry 1)* and `npm-publish-canary`
  *(entry 2)*

Use the **"pending publisher"** option so it works for first-publish of
each package. Pending publishers expire (currently ~30 days on npm), so
configure close to first publish attempt.
...
- [ ] Create `npm-publish-stable` with required reviewer(s); optionally
  restrict to `main` branch via deployment branch rules
- [ ] Create `npm-publish-canary` (empty — just needs to exist so the
  trusted-publisher match works)
- [ ] Narrow / rotate `MIDNIGHTCI_PACKAGES_WRITE` PAT — now only needs
  `read:packages` scope (CD no longer writes to GH Packages)
...
- [ ] If OIDC fails, error log shows the `workflow_ref` value npm
  received — match the trusted-publisher config to that string exactly
...
- [ ] After 2-3 clean releases, convert this doc to steady-state
  publishing docs or delete
