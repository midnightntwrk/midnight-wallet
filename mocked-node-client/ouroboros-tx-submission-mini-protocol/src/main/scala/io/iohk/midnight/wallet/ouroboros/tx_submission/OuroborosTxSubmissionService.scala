...
import io.iohk.midnight.wallet.ouroboros.tx_submission.OuroborosTxSubmissionService.Error.{
  DeferredFailed,
...
/** Implementation of the TxSubmissionService
  ...
  */
class OuroborosTxSubmissionService[F[_]: GenConcurrentThrow, Transaction: Encoder: Show](
    webSocketClient: JsonWebSocketClient[F],
    pendingSubmissions: Queue[F, Deferred[F, SubmissionResult]],
    semaphore: Semaphore[F],
)(implicit tracer: OuroborosTxSubmissionTracer[F]) {
  ...
  def submitTransaction(transaction: Transaction): F[SubmissionResult] =
    for {
      ...
      _ <- webSocketClient.send[LocalTxSubmission.Send[Transaction]](SubmitTx(transaction))
...
object OuroborosTxSubmissionService {
  ...
  def apply[F[_]: GenConcurrentThrow, Transaction: Encoder: Show](
      webSocketClient: JsonWebSocketClient[F],
  )(implicit
      tracer: OuroborosTxSubmissionTracer[F],
  ): Resource[F, OuroborosTxSubmissionService[F, Transaction]] = {
    ...
    val instance =
      (submissionsQueue, semaphore).mapN(
        new OuroborosTxSubmissionService[F, Transaction](webSocketClient, _, _),
      )
