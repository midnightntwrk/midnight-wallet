...
# Include both successful and failed runs, as failed runs may still have useful sync cache artifacts
runs=$(gh api repos/$REPO/actions/workflows/$workflow_id/runs --paginate | jq -r '.workflow_runs[] | select(.status == "completed" and (.conclusion == "success" or .conclusion == "failure")) | .id')
