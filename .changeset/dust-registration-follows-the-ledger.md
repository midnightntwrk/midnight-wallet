...
what the ledger says of them after the fork, and what the indexer reports for them — it scopes the flag to the
chain's current dust epoch, so a UTxO whose generation the fork wiped reads `false`. The indexer never
