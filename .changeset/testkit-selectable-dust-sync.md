...
`eventLessDustWallet` is a single-variant V2 composition, which is what reaches the projections sync directly on a
chain that runs ledger-v9 from its first block.
...
`facade.doSync(seeds)` at every point that would otherwise wait for background convergence — swapping only
