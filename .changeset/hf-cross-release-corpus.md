...
test(wallets): replay snapshots written by the last pre-fork release
...
`scripts/cross-release-corpus` pins the last pre-fork SDK release, generates fixtures with it, and writes them into
...
Mutation-verified: a reader that stops accepting the pre-fork creation-time encoding leaves **257 of 261** unit tests
