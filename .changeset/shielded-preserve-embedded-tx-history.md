...
The field is now read back into the wallet and written out untouched, by both variants, and it crosses `forks.v9` with
the rest of the wallet, so a snapshot saved after the crossing still carries it. The SDK neither reads nor adds to it; it only
